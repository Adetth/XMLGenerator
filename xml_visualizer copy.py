...
def generate_excel_report(root, output_file=f"XML_Diagnostic_Report{INPUT_XML_FILE}.xlsx"):
    """
    Generates a rich Excel report with visual color feedback.
    """
    ...
    fmts = get_form_formatting_container(root)
    colors = get_color_containers(root) # Useful to show the palette
    ...
    # ==========================================
    # SHEET 1: ROWS & COLUMNS
    # ==========================================
    ws_struct = wb.active
    ws_struct.title = "Structure (Rows & Cols)"
    setup_sheet(ws_struct, ["Category", "Name", "Type", "Size", "Formula/Member Details"])
    
    current_row = 2
    
    # Write Rows
    for r in rows:
        r_type = r.formula_or_function.name if r.formula_or_function else "UNKNOWN"
        
        # LOGIC CHANGE: Force the "Function : Member" format
        if r.formula_or_function == DimType.FORMULA:
            details = f"Label : {r.formula_label}"
            if r.formula_equation:
                 details += f" | Eq : {r.formula_equation}"
        else:
            # This produces "Descendants : YearTotal"
            details = f"{r.function_name} : {r.function_member_name}"
        
        ws_struct.append(["Row", r.name, r_type, r.size, details])
        current_row += 1
    ...
    # Write Cols
    for c in cols:
        c_type = c.formula_or_function.name if c.formula_or_function else "UNKNOWN"
        
        if c.formula_or_function == DimType.FORMULA:
            details = f"Label : {c.formula_label}"
            if c.formula_equation:
                 details += f" | Eq : {c.formula_equation}"
        else:
             # This produces "Descendants : YearTotal"
            details = f"{c.function_name} : {c.function_member_name}"
        
        ws_struct.append(["Column", c.name, c_type, c.size, details])
        current_row += 1

    # ==========================================
    # SHEET 2: VALIDATION RULES (VISUALS)
    # ==========================================
    ws_vals = wb.create_sheet("Grid Rules (Visual)")
    setup_sheet(ws_vals, ["Rule Name", "Row Loc", "Col Loc", "Style ID", "Hex Code", "COLOR PREVIEW"])
    
    for v in vals:
        # Create Row Data
        row_data = [v.name, v.row_location, v.col_location, v.style_id, v.bg_color, ""]
        ws_vals.append(row_data)
        
        # PAINT THE CELL
        # v.bg_color is a hex string like "FF8C00". OpenPyXL usually likes "AARRGGBB".
        # We prepend "FF" for full opacity if it's 6 chars long.
        if v.bg_color:
            try:
                clean_hex = v.bg_color.replace("#", "")
                if len(clean_hex) == 6:
                    clean_hex = "FF" + clean_hex
                
                fill = PatternFill(start_color=clean_hex, end_color=clean_hex, fill_type="solid")
                
                # Apply to the "COLOR PREVIEW" column (Col 6)
                cell = ws_vals.cell(row=ws_vals.max_row, column=6)
                cell.fill = fill
                cell.value = "   " # Empty text, just color
            except Exception:
                pass # Ignore invalid colors

    # ==========================================
    # SHEET 3: LOGICAL FORMATTING
    # ==========================================
    ws_fmt = wb.create_sheet("Logical Formatting")
    setup_sheet(ws_fmt, ["Style ID", "Row Name", "Row Dim", "Col Name", "Col Dim"])
    
    for f in fmts:
        ws_fmt.append([f.style_id, f.row_name, f.row_dim, f.col_name, f.col_dim])

    # ==========================================
    # SHEET 4: COLOR PALETTE
    # ==========================================
    ws_colors = wb.create_sheet("Color Palette")
    setup_sheet(ws_colors, ["Color ID", "RGB", "Hex Code", "PREVIEW"])
    
    for c in colors:
        try:
            r, g, b = int(c.red), int(c.green), int(c.blue)
            
            # FIX: :02X ensures 5 becomes "05", not "5"
            hex_val = f"{r:02X}{g:02X}{b:02X}"
            argb_val = "FF" + hex_val
            
            ws_colors.append([c.color_id, f"{r},{g},{b}", hex_val, ""])
            
            # Paint Preview
            fill = PatternFill(start_color=argb_val, end_color=argb_val, fill_type="solid")
            cell = ws_colors.cell(row=ws_colors.max_row, column=4)
            cell.fill = fill
        except:
            ws_colors.append([c.color_id, "Error", "Error", ""])
   
    
    # Save
    wb.save(output_file)
    print(f"Done! Open '{output_file}' to see the results.")

# --- EXECUTE ---
if __name__ == "__main__":
    generate_excel_report(root)